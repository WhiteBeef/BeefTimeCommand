package ru.whitebeef.beeftimecommand;
...
public class Main extends JavaPlugin implements Runnable {
	...
	public List<String> getCommand(LocalTime time) {
		...
		if (CONFIG_MODE == 1)
			return getConfig().getStringList("CommandsTime." + hour + ":" + time.getMinute());
		if (CONFIG_MODE == 2) {
			LocalDateTime dateTime = LocalDateTime.now();
			return getConfig().getStringList("CommandsTime." + dateTime.getDayOfWeek().toString().toLowerCase() + "."
					+ hour + ":" + time.getMinute());
		}
